...
function createContentLoader(pattern, { includeSrc, render, excerpt: renderExcerpt, transform } = {}) {
  ...
  return {
    ...
    async load(files) {
      if (!files) {
        files = (
          await init.glob(pattern, {
            ignore: ['**/node_modules/**', '**/dist/**']
          })
        ).sort();
      }
      md = md || (await init.createMarkdownRenderer(config.srcDir, config.markdown, config.site.base, config.logger));
      const raw = [];
