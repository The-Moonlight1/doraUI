import { onMounted, onBeforeUnmount, ref, watch, reactive, useSlots, defineComponent, h } from 'vue';
...
export function usePosition(tooltip: Ref<HTMLElement | null | undefined>) {
  ...
  useObserver(
    ...
    e => {
      ...
      // 判断左右超出状态
      if ((left <= bl && right >= br) || (left == 0 && right == 0)) position.value[0] = '';
      else if (left <= bl) position.value[0] = 'left';
      else if (right >= br) position.value[0] = 'right';
      else position.value[0] = '';

      // 判断上下超出状态
      if ((bottom >= bb && top <= bt) || (top == 0 && bottom == 0)) position.value[1] = '';
 ...
 * */
export function usePlacement(tooltip: Ref<HTMLElement | null | undefined>, placement: TooltipProps['placement']) {
...
export function useDefaultSlots() {
  ...
  const DefaultSlot = defineComponent((componentProps, context) => {
    return () => {
      const VNodes = slots.default ? slots.default() : [h('span', {}, 'tooltip')];
      if (VNodes.length > 1)
        console.error(new Error('Component can only have one root element, but you have used multiple root elements'));
